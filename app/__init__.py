...
def create_app(config_object=Config):
    ...
    # Make shared flags available to every template.
    @app.context_processor
    def inject_flags():
        from .roles import user_is_officer
        return {
            "google_enabled": app.config.get("GOOGLE_ENABLED", False),
            "is_officer": user_is_officer(),
            "storage_enabled": app.config.get("STORAGE_ENABLED", False),
        }
    ...
    from .blueprints.ai_routes import bp as ai_bp
    from .blueprints.logs_routes import bp as logs_bp
