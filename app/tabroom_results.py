...
def scrape_record(url):
    ...
    # Tournament name: first <h2>/<h3>, else <title>.
    name = ""
    m = re.search(r"<h[23][^>]*>(.*?)</h[23]>", raw, re.S)
    if m:
        name = _clean(m.group(1))[:120]

    # Prefer an explicit "Record 4-2" style; else count Win/Loss words.
    wins = losses = 0
    rec = re.search(r"[Rr]ecord[:\s]+(\d+)\s*[-–]\s*(\d+)", text)
    if rec:
        wins, losses = int(rec.group(1)), int(rec.group(2))
    else:
        # Count round outcomes. Tabroom often marks results as W / L or Win / Loss.
